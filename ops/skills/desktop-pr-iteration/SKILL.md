...
- CodeQL may flag imported renderer media sinks after a web pull. Keep URL handling real, not cosmetic: sanitize audio/video source URLs through an allowlist before assigning `src`, remove unnecessary clickable links for untrusted iframe banners, and only suppress a CodeQL finding when the local validator already pins protocol, host, path, and query parameters tightly enough that the remaining alert is a static-analysis false positive. If the OpenGraph route is flagged for JSON-LD image values, prefer dropping JSON-LD images as preview fetch candidates while preserving JSON-LD text metadata; OpenGraph and Twitter image tags still cover normal preview images without turning arbitrary JSON-LD fields into outbound image fetches.
