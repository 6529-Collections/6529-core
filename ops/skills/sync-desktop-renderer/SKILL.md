...
- `renderer/wagmiConfig/seedWalletConnector.ts`, `renderer/wagmiConfig/seedWalletConnectionState.ts`: each Core seed-wallet connector must accept persisted connection state only for its own address, register request callbacks before dispatch, and reject pending requests on disconnect.
