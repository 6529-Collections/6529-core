...
- `renderer/components/header/user/HeaderUserConnectModal.tsx`, `renderer/components/header/user/connector-modal-layout.ts`, `renderer/components/header/user/seed-wallet-selection-state.ts`: keep the connector chooser viewport-bounded but wider than the old 400px cap without increasing row height. Show `Connected · Active` on the disabled active Core wallet and `Connected · Switch` on another authenticated wallet; switching must reuse the authenticated account instead of starting a new connection.
- `renderer/components/confirm/ConfirmSeedWalletRequest.tsx`, `renderer/components/confirm/seed-wallet-request-layout.ts`: keep the Core request prompt viewport-bounded, wider and taller than the shared default, with only the request body scrolling while its header and actions remain fixed.
