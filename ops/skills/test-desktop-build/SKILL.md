...
Then install renderer dependencies from `renderer/`:
...
Run the smallest checks that prove the merge contract first, then broaden:
...
$env:SEIZE_6529_COMMAND='1'; pnpm exec electron-builder --config electron-builder.production.json --win --x64 --publish never
```
