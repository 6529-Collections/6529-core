...
awk \
  ...
  -v repo_end="$repo_marker_end" '
    BEGIN { skip = 0 }
    index($0, old_begin) == 1 { skip = 1; next }
    index($0, legacy_begin) == 1 { skip = 1; next }
    index($0, repo_begin) == 1 { skip = 1; next }
    skip && (index($0, old_end) == 1 || index($0, legacy_end) == 1 || index($0, repo_end) == 1) { skip = 0; next }
    !skip { print }
  ' "$rc_file" > "$tmp_file"
...
log "Configured repo-scoped \`6529\` command in $rc_file"
if [[ "$REMOVED_GLOBAL_SHIM" == "1" ]]; then
  log "Removed the old managed global shim at ${HOME}/.local/bin/6529"
fi
log "Open a new shell, or run:"
log "  source <(\"$REPO_ROOT/bin/6529\" bootstrap --print-export)"
log "Then install dependencies with:"
log "  6529 install"
