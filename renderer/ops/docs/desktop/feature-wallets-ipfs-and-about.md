...
When Electron detects a new desktop version, an Update rocket appears above
Search in the sidebar. Select it to open About and use the existing download
and install controls. It replaces the desktop update toast; it does not reload
the web page or download automatically. The App Info indicator remains available.
In local, dev, and staging builds, `?showDesktopUpdate=true` previews the sidebar
item (`showDesktopUpdateModal=true` remains supported). Production ignores these
preview parameters.
