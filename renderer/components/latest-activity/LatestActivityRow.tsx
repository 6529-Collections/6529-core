import { MANIFOLD } from "@/constants/constants";
import { NextGenCollection } from "@/entities/INextgen";
import { NFTLite } from "@/entities/INFT";
import { Transaction } from "@/entities/ITransaction";
import { getRandomObjectId } from "@/helpers/AllowlistToolHelpers";
import {
  areEqualAddresses,
  areEqualURLS,
  displayDecimal,
  getDateDisplay,
  getRoyaltyImage,
  isGradientsContract,
  isMemeLabContract,
  isMemesContract,
  isNextgenContract,
  isNullAddress,
  numberWithCommas,
} from "@/helpers/Helpers";
import {
  faCartPlus,
  faExchange,
  faExternalLinkSquare,
  faFire,
  faGasPump,
  faParachuteBox,
  faShoppingCart,
} from "@fortawesome/free-solid-svg-icons";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import Image from "next/image";
import Link from "next/link";
import { Tooltip } from "react-tooltip";
import Address from "../address/Address";
import {
  getNextGenIconUrl,
  getNextGenImageUrl,
} from "../nextGen/collections/nextgenToken/NextGenTokenImage";
import { NEXTGEN_CHAIN_ID, NEXTGEN_CORE } from "../nextGen/nextgen_contracts";
import { normalizeNextgenTokenID } from "../nextGen/nextgen_helpers";

function calculateRoyaltiesPercentage(value: number, royalties: number) {
  return Math.round((royalties / value) * 10000) / 10000;
}

export function printRoyalties(
  value: number,
  royalties: number,
  from: string,
  iconHeight = "25px"
) {
  if (value == 0 || isNullAddress(from) || areEqualAddresses(from, MANIFOLD)) {
    return <></>;
  }
  const royaltiesPercentage = calculateRoyaltiesPercentage(value, royalties);
  if (royalties <= 0) return <></>;

  const imgSrc = getRoyaltyImage(royaltiesPercentage);
  const id = getRandomObjectId();

  return (
    <>
      <Image
        unoptimized
        data-tooltip-id={id}
        width={0}
        height={0}
        style={{ height: iconHeight, width: "auto" }}
        src={`/${imgSrc}`}
        alt={imgSrc}
        className="tw-cursor-pointer"
      />
      <Tooltip
        id={id}
        delayShow={150}
        place="top-end"
        opacity={1}
        variant="light"
        className="tw-leading-tight"
      >
        <div className="tw-flex tw-gap-3">
          <span>Royalties</span>
          <span>
            {royalties > 0
              ? `${displayDecimal(royalties)} (${displayDecimal(
                  royaltiesPercentage * 100
                )}%)`
              : "-"}
          </span>
        </div>
      </Tooltip>
    </>
  );
}

export function printGas(
  gas: number,
  gas_gwei: number,
  gas_price_gwei: number,
  iconClassName = "tw-h-5 tw-w-5 tw-cursor-pointer tw-text-iron-400 tw-transition-colors hover:tw-text-white"
) {
  const id = getRandomObjectId();
  return (
    <>
      <FontAwesomeIcon
        data-tooltip-id={id}
        className={iconClassName}
        icon={faGasPump}
      ></FontAwesomeIcon>
      <Tooltip
        id={id}
        delayShow={150}
        place="top-end"
        opacity={1}
        variant="light"
        className="tw-leading-tight"
      >
        <div className="tw-flex tw-flex-col tw-gap-1">
          {[
            ["Gas", `${displayDecimal(gas)}`],
            ["GWEI", numberWithCommas(gas_gwei)],
            ["Gas Price", `${displayDecimal(gas_price_gwei)}`],
          ].map(([label, value]) => (
            <div
              key={label}
              className="tw-flex tw-w-full tw-justify-between tw-gap-4"
            >
              <span className="tw-w-1/2 tw-text-left">{label}</span>
              <span className="tw-w-1/2 tw-text-right">{value}</span>
            </div>
          ))}
        </div>
      </Tooltip>
    </>
  );
}

interface Props {
  nft?: NFTLite | undefined;
  nextgen_collection?: NextGenCollection | undefined;
  tr: Transaction;
  hideNextgenTokenId?: boolean | undefined;
  showNftIdentity?: boolean | undefined;
}

export default function LatestActivityRow(props: Readonly<Props>) {
  const nftImageClassName = "tw-inline-block tw-rounded-sm";
  const activityIconClassName =
    "tw-h-5 tw-w-5 tw-cursor-pointer tw-text-iron-400 tw-transition-colors hover:tw-text-white";
  const activityGasIconClassName =
    "tw-h-4 tw-w-4 tw-cursor-pointer tw-text-iron-400 tw-transition-colors hover:tw-text-white";

  function getNftImageSrc(nft?: NFTLite, src?: string) {
    if (!nft) {
      return "";
    }
    if (!src) {
      return nft.icon;
    }
    if (areEqualURLS(src, nft.icon)) {
      return nft.thumbnail;
    }
    if (areEqualURLS(src, nft.thumbnail)) {
      return nft.scaled;
    }
    if (areEqualURLS(src, nft.scaled)) {
      return nft.image;
    }
    return "";
  }

  function getHref() {
    if (isMemesContract(props.tr.contract)) {
      return `/the-memes/${props.tr.token_id}`;
    }
    if (isGradientsContract(props.tr.contract)) {
      return `/6529-gradient/${props.tr.token_id}`;
    }
    if (isMemeLabContract(props.tr.contract)) {
      return `/meme-lab/${props.tr.token_id}`;
    }
    if (isNextgenContract(props.tr.contract)) {
      return `/nextgen/token/${props.tr.token_id}/provenance`;
    }
    return `#${props.tr.token_id}`;
  }

  function getNftLabel() {
    if (isMemesContract(props.tr.contract)) {
      return `The Memes #${props.tr.token_id}`;
    }
    if (isMemeLabContract(props.tr.contract)) {
      return `MemeLab #${props.tr.token_id}`;
    }
    if (isGradientsContract(props.tr.contract)) {
      return `6529 Gradient #${props.tr.token_id}`;
    }
    if (isNextgenContract(props.tr.contract)) {
      const normalized = normalizeNextgenTokenID(props.tr.token_id);
      const collectionName =
        props.nextgen_collection?.name ??
        `NextGen #${normalized.collection_id}`;
      return `${collectionName} #${normalized.token_id}`;
    }
    return `#${props.tr.token_id}`;
  }

  function printNftImage() {
    if (isNextgenContract(props.tr.contract)) {
      return (
        <Link
          href={getHref()}
          className="tw-inline-flex tw-h-10 tw-w-12 tw-items-center tw-justify-center tw-overflow-hidden"
        >
          <Image
            unoptimized
            width={0}
            height={0}
            style={{ height: "100%", width: "100%", objectFit: "contain" }}
            src={getNextGenIconUrl(props.tr.token_id)}
            alt={getNftLabel()}
            className={nftImageClassName}
            onError={({ currentTarget }) => {
              if (currentTarget.src === getNextGenIconUrl(props.tr.token_id)) {
                currentTarget.src = getNextGenImageUrl(props.tr.token_id);
              }
            }}
          />
        </Link>
      );
    }

    if (props.nft) {
      return (
        <Link
          href={getHref()}
          className="tw-inline-flex tw-h-10 tw-w-12 tw-items-center tw-justify-center tw-overflow-hidden"
        >
          <Image
            unoptimized
            width={0}
            height={0}
            style={{ height: "100%", width: "100%", objectFit: "contain" }}
            src={getNftImageSrc(props.nft)}
            alt={props.nft.name}
            onError={({ currentTarget }) => {
              currentTarget.src = getNftImageSrc(props.nft, currentTarget.src);
            }}
            className={nftImageClassName}
          />
        </Link>
      );
    }

    return null;
  }

  function printNftIdentity() {
    return (
      <Link
        href={getHref()}
        className="tw-font-semibold tw-text-white tw-underline tw-underline-offset-2 tw-transition-opacity hover:tw-text-white hover:tw-opacity-80"
      >
        {getNftLabel()}
      </Link>
    );
  }

  function printTokenDetails() {
    const showQuantity =
      isMemesContract(props.tr.contract) ||
      isMemeLabContract(props.tr.contract);

    if (!showQuantity && !props.showNftIdentity) {
      return null;
    }

    return (
      <>
        {showQuantity && (
          <>
            {props.tr.token_count}x{props.showNftIdentity && <>&nbsp;</>}
          </>
        )}
        {props.showNftIdentity && printNftIdentity()}
      </>
    );
  }

  function printDescription() {
    const tokenDetails = printTokenDetails();

    return (
      <span className="tw-flex tw-items-center tw-whitespace-nowrap">
        {(areEqualAddresses(MANIFOLD, props.tr.from_address) ||
          isNullAddress(props.tr.from_address)) && (
          <>
            {props.tr.value > 0 ? (
              <>
                <Address
                  wallets={[props.tr.to_address]}
                  display={props.tr.to_display}
                />
                seized
              </>
            ) : (
              "Airdrop"
            )}
            {tokenDetails && <>&nbsp;{tokenDetails}</>}
            {props.tr.value === 0 && (
              <>
                &nbsp;to&nbsp;
                <Address
                  wallets={[props.tr.to_address]}
                  display={props.tr.to_display}
                />
              </>
            )}
            &nbsp;&nbsp;
            {props.tr.value > 0 &&
              `for ${Math.round(props.tr.value * 100000) / 100000} ETH`}
          </>
        )}
        {isNullAddress(props.tr.to_address) && (
          <>
            <Address
              wallets={[props.tr.from_address]}
              display={props.tr.from_display}
            />
            {" burnt"}
            {tokenDetails && <>&nbsp;{tokenDetails}</>}
            &nbsp;
          </>
        )}
        {!areEqualAddresses(MANIFOLD, props.tr.from_address) &&
          !isNullAddress(props.tr.from_address) &&
          !isNullAddress(props.tr.to_address) && (
            <>
              <Address
                wallets={[props.tr.to_address]}
                display={props.tr.to_display}
              />
              &nbsp;
              {props.tr.value > 0 ? "bought" : "received"}
              {tokenDetails && <>&nbsp;{tokenDetails}</>}
              &nbsp;from&nbsp;
              <Address
                wallets={[props.tr.from_address]}
                display={props.tr.from_display}
              />
              {props.tr.value > 0 &&
                ` for ${Math.round(props.tr.value * 100000) / 100000} ETH`}
            </>
          )}
      </span>
    );
  }

  function getDescription() {
    const normalized = normalizeNextgenTokenID(props.tr.token_id);
    const collectionName = props.nextgen_collection?.name ?? (
      <>NextGen #{normalized.collection_id}</>
    );
    const tokenInfo = (
      <a
        href={`/nextgen/token/${props.tr.token_id}/provenance`}
        target="_blank"
        rel="noopener noreferrer"
      >
        {collectionName} #{normalized.token_id}
        <Image
          unoptimized
          width={0}
          height={0}
          style={{
            height: "40px",
            width: "auto",
            marginLeft: "8px",
            marginRight: "8px",
          }}
          src={getNextGenIconUrl(props.tr.token_id)}
          alt={`#${props.tr.token_id.toString()}`}
          className={nftImageClassName}
          onError={({ currentTarget }) => {
            if (currentTarget.src === getNextGenIconUrl(props.tr.token_id)) {
              currentTarget.src = getNextGenImageUrl(props.tr.token_id);
            }
          }}
        />
      </a>
    );

    if (isNullAddress(props.tr.from_address)) {
      return (
        <>
          {!props.hideNextgenTokenId ? <>{tokenInfo} seized by</> : `Seized by`}
          &nbsp;
          <Address
            wallets={[props.tr.to_address]}
            display={props.tr.to_display}
          />
        </>
      );
    }

    if (isNullAddress(props.tr.to_address)) {
      return (
        <>
          {!props.hideNextgenTokenId ? <>{tokenInfo} burnt by</> : `Burnt by`}
          <Address
            wallets={[props.tr.from_address]}
            display={props.tr.from_display}
          />
        </>
      );
    }

    if (props.tr.value === 0) {
      return (
        <>
          {!props.hideNextgenTokenId ? (
            <>{tokenInfo} transferred from</>
          ) : (
            "Transferred from"
          )}
          &nbsp;
          <Address
            wallets={[props.tr.from_address]}
            display={props.tr.from_display}
          />
          &nbsp;to&nbsp;
          <Address
            wallets={[props.tr.to_address]}
            display={props.tr.to_display}
          />
        </>
      );
    }

    return (
      <>
        {!props.hideNextgenTokenId ? (
          <>{tokenInfo} purchased by</>
        ) : (
          "Purchased by"
        )}
        &nbsp;
        <Address
          wallets={[props.tr.to_address]}
          display={props.tr.to_display}
        />
        from&nbsp;
        <Address
          wallets={[props.tr.from_address]}
          display={props.tr.from_display}
        />
      </>
    );
  }

  function printDescriptionNextgen() {
    return (
      <span className="tw-flex tw-items-center tw-whitespace-nowrap">
        {getDescription()}
        {props.tr.value > 0 &&
          ` for ${Math.round(props.tr.value * 100000) / 100000} ETH`}
      </span>
    );
  }

  function printInfo() {
    return (
      <span className="tw-flex tw-items-center tw-gap-3 tw-pl-4">
        {printRoyalties(
          props.tr.value,
          props.tr.royalties,
          props.tr.from_address,
          "22px"
        )}
        {printGas(
          props.tr.gas,
          props.tr.gas_gwei,
          props.tr.gas_price_gwei,
          activityGasIconClassName
        )}
        <Link
          href={`https://etherscan.io/tx/${props.tr.transaction}`}
          target="_blank"
          rel="noopener noreferrer"
          className="tw-flex"
        >
          <FontAwesomeIcon
            className={activityIconClassName}
            icon={faExternalLinkSquare}
          ></FontAwesomeIcon>
        </Link>
      </span>
    );
  }

  function isBurn() {
    return isNullAddress(props.tr.to_address);
  }

  function isMint() {
    return (
      areEqualAddresses(MANIFOLD, props.tr.from_address) ||
      isNullAddress(props.tr.from_address)
    );
  }

  function isAirdrop() {
    return isNullAddress(props.tr.from_address);
  }

  function getIconClass() {
    const baseClassName = "tw-h-5 tw-w-5";
    if (isBurn()) {
      return `${baseClassName} tw-text-red`;
    }
    if (isMint() || isAirdrop()) {
      return `${baseClassName} tw-text-iron-100`;
    }
    if (props.tr.value > 0) {
      return `${baseClassName} tw-text-primary-300`;
    }
    return `${baseClassName} tw-text-success`;
  }

  function getIcon() {
    if (isBurn()) {
      return faFire;
    }

    if (props.tr.value > 0) {
      return isMint() ? faCartPlus : faShoppingCart;
    }

    if (isAirdrop()) {
      return faParachuteBox;
    }

    return faExchange;
  }

  if (!props.tr.token_count) {
    return <></>;
  }

  const tailwindCellBorderClassName = "tw-border-b tw-border-iron-800";

  return (
    <tr
      key={`${props.tr.from_address}-${props.tr.to_address}-${props.tr.transaction}-${props.tr.token_id}-latestactivity-row`}
      className="tw-h-16 odd:tw-bg-transparent even:tw-bg-iron-900/45 hover:tw-bg-iron-900/70"
    >
      <td
        className={`${props.showNftIdentity ? "" : "tw-w-px"} tw-whitespace-nowrap tw-border-0 ${tailwindCellBorderClassName} tw-border-solid tw-px-4 tw-py-3 tw-align-middle tw-text-sm tw-font-medium tw-leading-5 tw-text-iron-400`}
      >
        {getDateDisplay(new Date(props.tr.transaction_date))}
      </td>
      <td
        className={`tw-w-14 tw-whitespace-nowrap tw-border-0 ${tailwindCellBorderClassName} tw-border-solid tw-px-4 tw-py-3 tw-text-center tw-align-middle`}
      >
        <FontAwesomeIcon className={getIconClass()} icon={getIcon()} />
      </td>
      {props.showNftIdentity && (
        <td
          className={`tw-w-20 tw-whitespace-nowrap tw-border-0 ${tailwindCellBorderClassName} tw-border-solid tw-px-4 tw-py-3 tw-text-center tw-align-middle`}
        >
          {printNftImage()}
        </td>
      )}
      <td
        className={`tw-whitespace-nowrap tw-border-0 ${tailwindCellBorderClassName} tw-border-solid tw-px-4 tw-py-3 tw-align-middle tw-text-sm tw-font-medium tw-leading-5 tw-text-white`}
      >
        <div className="tw-flex tw-items-center tw-justify-between tw-gap-2">
          {areEqualAddresses(
            props.tr.contract,
            NEXTGEN_CORE[NEXTGEN_CHAIN_ID]
          ) && !props.showNftIdentity
            ? printDescriptionNextgen()
            : printDescription()}
          {printInfo()}
        </div>
      </td>
    </tr>
  );
}
