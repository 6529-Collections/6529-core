...
type SignInSessionResult = {
  readonly response: AuthSessionResponse | null;
...
const createSignInSession = async ({
  ...
  AuthRequestSignInParams): Promise<SignInSessionResult> => {
  ...
  if (clientSignature.userRejected) {
    ...
    return { response: null, cancelled: false };
  }
  ...
  return isAuthRequestStale(authRequestGuard)
    ? { response: null, cancelled: true }
    : { response: sessionResponse, cancelled: false };
};
...
export const createAuthRequestSignIn =
  ...
  async ({
    ...
    role,
    signerAddress,
  }: AuthRequestSignInParams): Promise<{
    success: boolean;
    ...
    try {
      ...
      const signInSession = await createSignInSession({
        authRequestGuard,
        getNonce,
        getSignature,
        role,
      ...
      if (!signInSession.response) {
        return {
          success: false,
          cancelled: signInSession.cancelled || undefined,
        };
      }

      const isPersisted = await persistSessionResponse(
        signInSession.response,
        {
