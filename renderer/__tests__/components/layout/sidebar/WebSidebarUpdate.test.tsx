...
it("puts the collapsed update rocket above Search with no desktop toast", () => {
  ...
  expect(mockPush).toHaveBeenCalledWith("/core/core-info");
...
it.each([
  ...
  { isAppleMobile: true },
  { hasTouchScreen: true },
])("keeps the Core sidebar update on touch-capable hardware: %j", (device) => {
  jest.mocked(useDeviceInfo).mockReturnValue({
    isApp: false,
    isMobileDevice: false,
    isAppleMobile: false,
  ...
  expect(screen.queryByRole("button", { name: "Update" })).toBeInTheDocument();
});
