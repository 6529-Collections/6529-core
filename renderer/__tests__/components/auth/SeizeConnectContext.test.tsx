...
const TestComponent: React.FC = () => {
  ...
  return (
    <div data-testid="test-component">
      ...
      <button onClick={handleAcceptValid} data-testid="accept-valid">
        Accept Valid
      </button>
      <button onClick={handleAcceptInvalid} data-testid="accept-invalid">
        Accept Invalid
      </button>
...
const renderWithProvider = (children: React.ReactNode = <TestComponent />) => {
  return render(<SeizeConnectProvider>{children}</SeizeConnectProvider>);
};
...
describe("SeizeConnectContext Security Logging", () => {
  ...
  describe("Error Logging in Production", () => {
    ...
    it("logs errors in production mode with sanitization", async () => {
      ...
      const mockOpen = jest.fn().mockImplementation(() => {
        throw new Error(
          "Connection failed with wallet 0x1234567890123456789012345678901234567890"
        );
      });
      ...
      const logCall = consoleErrorSpy.mock.calls.find(
        (call) => call[0] === "[SEIZE_CONNECT_ERROR]"
      );
    ...
    it("sanitizes potential tokens in error messages", async () => {
      ...
      const mockOpen = jest.fn().mockImplementation(() => {
        throw new Error(
          "Token abcdef1234567890abcdef1234567890abcdef12 is invalid"
        );
      });
      useAppKit.mockReturnValue({ open: mockOpen });
  ...
  describe("Error Logging in Development", () => {
      ...
      expect(loggedData).toEqual(
        expect.objectContaining({
          ...
          stack: expect.stringContaining(
            "WalletConnectionError: Failed to open wallet connection modal"
          ),
    ...
    it("sanitizes sensitive data in development mode cause field", async () => {
      ...
      const mockOpen = jest.fn().mockImplementation(() => {
        throw new Error(
          "Connection failed with wallet 0x1234567890123456789012345678901234567890 and token abcdef1234567890abcdef1234567890abcdef12"
        );
      });
      useAppKit.mockReturnValue({ open: mockOpen });
      ...
      expect(loggedData.cause).not.toContain(
  ...
  describe("Security Event Logging", () => {
    ...
    it("never logs raw addresses in production", async () => {
      ...
      expect(callsAsString).not.toContain(
        "0x1234567890123456789012345678901234567890"
      );
  ...
  describe("Logging Never Disabled", () => {
    ...
    it("security events require explicit enablement even with undefined NODE_ENV", async () => {
      const { publicEnv } = require("@/config/env");
      publicEnv.NODE_ENV = undefined;
      publicEnv.ENABLE_SECURITY_LOGGING = undefined;
...
describe("SeizeConnectContext Security Vulnerability Fix", () => {
  let mockGetWalletAddress: jest.MockedFunction<
  ...
  beforeEach(() => {
    mockGetWalletAddress = authUtils.getWalletAddress as jest.MockedFunction<
      typeof authUtils.getWalletAddress
    >;
    mockRemoveAuthJwt = authUtils.removeAuthJwt as jest.MockedFunction<
  ...
  describe("Critical Security Fix: useState Initialization", () => {
    it("should NOT throw during initial render with invalid stored address", async () => {
      ...
      try {
        expect(
          screen.getByText("Initializing wallet connection...")
        ).toBeInTheDocument();
      } catch {
  ...
  describe("Error Boundary Functionality", () => {
    it("should catch and display wallet initialization errors", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(/Something went wrong with your wallet connection/)
        ).toBeInTheDocument();
